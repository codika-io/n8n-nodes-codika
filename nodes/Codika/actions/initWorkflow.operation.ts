...
import { CODIKA_API_URL } from '../shared/executionUtils';
...
export const initWorkflowDescription: INodeProperties[] = [
	{
		...
		displayOptions,
		description:
			'This operation auto-detects the trigger type. For HTTP triggers (from Codika UI), it extracts execution metadata from the payload (passthrough mode). For other triggers (schedule, Gmail, etc.), it creates a new execution via API using the parameters below.',
	},
	{
		...
		displayOptions,
		description:
			'Required for non-HTTP triggers. Codika member execution auth secret (use placeholder: {{MEMSECRT_EXECUTION_AUTH_TRCESMEM}}).',
	},
	{
		...
		displayOptions,
		description:
			'Required for non-HTTP triggers. Codika organization ID (use placeholder: {{USERDATA_ORGANIZATION_ID_ATADRESU}}).',
	},
	{
		displayName: 'User ID',
		name: 'userId',
		...
		displayOptions,
		description:
			'Required for non-HTTP triggers. Codika user ID (use placeholder: {{USERDATA_USER_ID_ATADRESU}}).',
	},
	{
		...
		displayOptions,
		description:
			'Required for non-HTTP triggers. Codika process instance ID (use placeholder: {{USERDATA_PROCESS_INSTANCE_UID_ATADRESU}}).',
	},
	{
...
export async function executeInitWorkflow(
		...
		try {
			...
			if (executionContext?.customData) {
				...
				executionContext.customData.set(
					'codikaExecutionSecret',
					httpTriggerMetadata.executionSecret,
				);
	...
	try {
		...
		try {
			...
			if (executionContext?.customData) {
				...
				executionContext.customData.set(
					'codikaExecutionSecret',
					response.executionSecret as string,
				);
				executionContext.customData.set('codikaStartTimeMs', String(startTimeMs));
			}
		} catch {
